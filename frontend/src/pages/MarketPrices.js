...
const MarketPrices = () => {
  ...
  const [useLiveData, setUseLiveData] = useState(true); // Toggle for live vs mock data
  ...
  // Fetch market prices (live or regular based on toggle)
  const {
    data: marketData,
    ...
    queryKey: ['market-prices', selectedDistrict, selectedCrop, useLiveData],
    queryFn: () => useLiveData 
      ? ApiService.getLiveMarketPrices(selectedDistrict, selectedCrop)
      : ApiService.getMandiPrices(selectedDistrict, selectedCrop),
    staleTime: useLiveData ? 2 * 60 * 1000 : 5 * 60 * 1000, // 2 min for live, 5 min for regular
    onError: (error) => {
      console.error('Market prices error:', error);
      toast.error(`Failed to fetch ${useLiveData ? 'live' : 'regular'} market prices`);
    }
  });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-sm text-green-100">Mock Data</span>
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-green-300 focus:ring-offset-2 ${
                  useLiveData ? 'bg-green-400' : 'bg-gray-400'
                }`}
              >
                <span
            ...
            <div className="text-right">
              ...
              {marketData?.data?.data_source && (
                <p className="text-xs text-green-200 mt-1">
                  📊 Source: {marketData.data.data_source === 'real_government_apis' ? 'Government APIs' : 'Mock Data'}
                </p>
              )}
            </div>
          </div>
